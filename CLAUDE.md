...
Test-writing rules that matter here (rationale in docs/testing.md):
...
cargo test --release --test std_stable_subset    # "std gate": frontend-checks every std/ module
```
...
- `std/` — the Jinn standard library (`use math`; function calls are module-qualified, types are global). The alpha-stable subset and its policy: docs/std.md, docs/stability.md.
- `libjn/` — aspirational C-stdlib-in-Jinn; stub bodies, not part of std or the runtime.
...
- docs/jinn.md is the language tour and **every fenced example in it is compiled by tests/doc_examples.rs** — a doc edit that breaks an example fails the suite. Docs describe the language as implemented, with gaps stated inline.

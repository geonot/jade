...
- **[115]** (2026-06-16) [task 2-32-3-7] reexport-consent DROPPED (user directive); replaced with a self-declared visibility ceiling on the *target* package (scope.md §4). Two rungs: `public` (default, reach-in open) / `internal` (visible only within the target's own owner-scope subtree). Enforcement = ScopePath prefix check in a new `resolve_path_use` (pkgid.rs) that resolves multi-segment `use baz/bar` hop-by-hop then gates the final target; wired into Typer::resolve_scoped_path_use + UseLocal lowering for multi-segment imports. `visibility` manifest key parsed in ProjectConfig; threaded through dag.rs (resolve_node) + packages.rs into PackageRecord. Docs updated in lockstep: scope.md §4, config-blocks.md, lamp.md §5.7.1, lamp-prereqs.md. +11 tests (5 pkgid unit, 3 dag e2e via real project.jn manifests + 3 reused).
- **[113]** (2026-06-16 17:16) [task 2-32-3-6] path-scoped use resolution: local resolve_use query (pkgid.rs), no global arbitration; Typer::resolve_scoped_use wired into UseLocal lowering for single-segment imports; single-package fast path preserved. +4 tests
- **[112]** (2026-06-15 20:13) [task 7] fix ternary mixed-fallible-arm phi ICE + plain/fallible struct-return bugs. reconcile_ternary_arms propagates the fallible arm to ok-inner before the phi. Root-cause for the struct-return zeroing + non-exhaustive-I64 match: single-uppercase struct names parsed as Type::Param made fns spuriously generic; typer now pre-scans declared_type_names and excludes them from inferred type params (effective_type_params/is_generic_fn/normalize_generic_fn now instance methods). +3 conformance tests.
- **[111]** task 7: fixed three codegen/typer bugs found making log_ingest.jn idiomatic. (1) Ternary `cond ? <plain> ! <fallible-call>` mixing an i64 arm with a Result<i64,E> arm caused a phi-type-mismatch ICE; typer now reconciles mismatched ternary arms inside a fallible fn by propagating the fallible arm to its ok-inner type (new `reconcile_ternary_arms` in expr/quaternary.rs, called from expr/mod.rs ternary lowering). (2)+(3) Root cause shared: a fn `*mk returns P` (or `returns P ! E`) where `P` is a single-uppercase-letter struct was mis-classified as generic — the parser turns any 1-char uppercase type name into `Type::Param`, so `mk` got monomorphized as `mk__G_i64` with return type collapsed to i64 (zeroed struct return; `match` on it seen as non-exhaustive I64). Fixed in the typer: pre-scan declared type/enum/err/actor/store names into `declared_type_names`; `effective_type_params`/`is_generic_fn`/`normalize_generic_fn` are now instance methods that exclude declared type names from inferred type params. +3 conformance tests (tests/error_effects.rs). Full suite green (3 pre-existing @column failures unrelated).
- **[110]** (2026-06-15 19:56) implicit error propagation in statement position: bare fallible expr-statements and bare `insert s a, b` now propagate (From-converted) + yield result like a bind, auto-wrapping Ok in tail position; no more `? $ !! err` ceremony on inserts. Typer StoreInsert/Expr lowering + block tail handling route through implicit_propagate; enclosing_fn_is_fallible made crate-visible. log_ingest land simplified. error-effects.md §5 clarified. +3 conformance tests. Removed stray tracked hits.ip.idx.
- **[110]** implicit error propagation in statement position: a bare fallible expression statement (`foo()`) and a bare store mutation (`insert s a, b`) now propagate like a bound `v is foo()` — on error the enclosing fallible fn returns (From-converted), on success they unwrap/yield the result, and as a tail expression auto-wrap to `Ok`. `insert s a, b` no longer needs `? $ !! err` ceremony. Typer: `Stmt::StoreInsert` lowering routes through `lower_expr_store_insert` + `implicit_propagate` when the enclosing fn is fallible (fire-and-forget otherwise, incl. main); `Stmt::Expr` runs `implicit_propagate`; block tail handling lowers fallible producers (calls/methods/inserts) as Result then propagates + auto-wraps, including a new `Stmt::StoreInsert` tail arm. `enclosing_fn_is_fallible` made crate-visible. log_ingest.jn `land` simplified to a bare insert. Docs: error-effects.md §5 clarified (uniform across bind/stmt/store positions). +3 conformance tests (tests/error_effects.rs). Removed stray tracked hits.ip.idx artifact.
- **[109]** (2026-06-15 19:28) examples/log_ingest.jn: maximally idiomatic rewrite — positional (unnamed) struct construction throughout (Hit/Report), replace if/else with unified ?/! conditional expression in bytes_of, drop redundant local in absorb. Also: completed task 2-32-3-6 wiring (fixed duplicate ScopedUseMap import, build clean zero warnings). Filed task 7 for 3 codegen bugs found (ternary mixed-fallible-arm phi ICE, bare struct-return zeroed, match-on-fallible-struct); recorded pitfalls in memory.

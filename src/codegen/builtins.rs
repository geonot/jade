...
impl<'ctx> Compiler<'ctx> {
    ...
    fn compile_fmt_float(&mut self, args: &[hir::Expr]) -> Result<BasicValueEnum<'ctx>, String> {
    ...
    fn compile_fmt_bin(&mut self, args: &[hir::Expr]) -> Result<BasicValueEnum<'ctx>, String> {
        let val = self.compile_expr(&args[0])?.into_int_value();
        let i64t = self.ctx.i64_type();
    ...
    fn compile_sleep_ms(&mut self, args: &[hir::Expr]) -> Result<BasicValueEnum<'ctx>, String> {
    ...
    fn compile_file_exists(&mut self, args: &[hir::Expr]) -> Result<BasicValueEnum<'ctx>, String> {

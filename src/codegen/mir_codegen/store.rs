...
impl<'ctx> Compiler<'ctx> {
    ...
    pub(super) fn emit_store_query(
        ...
        if use_index {
            ...
            if let Some(del_idx) = sd.fields.iter().position(|f| f.name == "deleted") {
                ...
                let del_val =
                    b!(self.bld.build_load(i64t, del_gep, "qi.del.val")).into_int_value();
    ...
    pub(super) fn emit_view_count(
        ...
        if let Some(del_idx) = sd.fields.iter().position(|f| f.name == "deleted") {
            ...
            let del_val =
                b!(self.bld.build_load(i64t, del_gep, "vc.del.val")).into_int_value();
            let is_deleted = b!(self.bld.build_int_compare(
                inkwell::IntPredicate::NE,
                del_val,
                i64t.const_int(0, false),
    ...
    pub(super) fn emit_view_all(
        ...
        if let Some(del_idx) = sd.fields.iter().position(|f| f.name == "deleted") {
            ...
            let del_val =
                b!(self.bld.build_load(i64t, del_gep, "va.del.val")).into_int_value();
            let is_deleted = b!(self.bld.build_int_compare(
                inkwell::IntPredicate::NE,
                del_val,
                i64t.const_int(0, false),
    ...
    pub(super) fn emit_store_all(&mut self, store_name: &str) -> Result<BasicValueEnum<'ctx>, String> {
        ...
        let jade_total = b!(self.bld.build_int_mul(
            count,
        ...
        if has_strings || deleted_idx.is_some() {
            ...
            if has_strings {
                ...
                let src_off = b!(self.bld.build_int_mul(
                    idx,
                    i64t.const_int(rec_size, false),
                    "all.soff"
                ));
            ...
            let total = b!(self.bld.build_int_mul(
                count,
                i64t.const_int(rec_size, false),
                "all.total"
            ));
    ...
    pub(super) fn emit_store_set(
        ...
        if let Some(del_idx) = sd.fields.iter().position(|f| f.name == "deleted") {
            ...
            let del_val =
                b!(self.bld.build_load(i64t, del_gep, "set.del.val")).into_int_value();
            let is_deleted = b!(self.bld.build_int_compare(
                inkwell::IntPredicate::NE,
                del_val,
                i64t.const_int(0, false),
        ...
        if is_versioned {
            ...
            if let (Some(si), Some(vi)) = (sid_idx, ver_idx) {
                ...
                let sid_val =
                    b!(self.bld.build_load(i64t, sid_gep, "ver.sid")).into_int_value();
                let ver_gep =
                    b!(self
                        .bld
                        .build_struct_gep(st, rec_ptr, vi as u32, "ver.ver.gep"));
                let old_ver =
                    b!(self.bld.build_load(i64t, ver_gep, "ver.old")).into_int_value();
    ...
    pub(super) fn emit_store_get(
        ...
        if let Some(del_idx) = sd.fields.iter().position(|f| f.name == "deleted") {
            ...
            let del_val =
                b!(self.bld.build_load(i64t, del_gep, "get.del.val")).into_int_value();
            let is_deleted = b!(self.bld.build_int_compare(
                inkwell::IntPredicate::NE,
                del_val,
                i64t.const_int(0, false),
        ...
        let rec_sid =
            b!(self.bld.build_load(i64t, rec_sid_gep, "get.sid.val")).into_int_value();
        let match_cond = b!(self.bld.build_int_compare(
            inkwell::IntPredicate::EQ,
            rec_sid,
            sid_val,
            "get.eq"
    ...
    pub(super) fn emit_store_exists(
        ...
        let fv = self.cur_fn.expect("ICE: cur_fn not set");
        let found_ptr = self
            .entry_alloca(self.ctx.bool_type().into(), "exists.found");
        b!(self
            .bld
        ...
        if let Some(del_idx) = sd.fields.iter().position(|f| f.name == "deleted") {
            ...
            let del_val =
                b!(self.bld.build_load(i64t, del_gep, "ex.del.val")).into_int_value();
            let is_deleted = b!(self.bld.build_int_compare(
                inkwell::IntPredicate::NE,
                del_val,
                i64t.const_int(0, false),

...
impl<'ctx> Compiler<'ctx> {
    ...
    pub(in crate::codegen) fn compute_enum_payload_offset(&self, enum_name: &str, target_idx: usize) -> u64 {
    ...
    pub(in crate::codegen) fn extract_coro_bodies_from_stmt(stmt: &hir::Stmt, out: &mut HashMap<Symbol, Vec<hir::Stmt>>) {
        match stmt {
    ...
    pub(in crate::codegen) fn extract_coro_bodies_from_expr(expr: &hir::Expr, out: &mut HashMap<Symbol, Vec<hir::Stmt>>) {
        match &expr.kind {

...
impl<'ctx> Compiler<'ctx> {
    ...
    pub(in crate::codegen) fn emit_kv_count(&mut self, store_name: &str) -> Result<BasicValueEnum<'ctx>, String> {
        let kv = self.load_kv_handle(store_name)?;
    ...
    pub(in crate::codegen) fn emit_ts_latest(&mut self, store_name: &str) -> Result<BasicValueEnum<'ctx>, String> {
    ...
    pub(in crate::codegen) fn emit_vec_count(&mut self, store_name: &str) -> Result<BasicValueEnum<'ctx>, String> {
        let sd = self
            .store_defs
            .get(store_name)
            .ok_or_else(|| format!("unknown store '{store_name}'"))?

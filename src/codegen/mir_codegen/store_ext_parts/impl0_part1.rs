...
impl<'ctx> Compiler<'ctx> {
    ...
    pub(in crate::codegen) fn emit_fts_count(&mut self, rest: &str) -> Result<BasicValueEnum<'ctx>, String> {
    ...
    pub(in crate::codegen) fn emit_store_distinct(&mut self, rest: &str) -> Result<BasicValueEnum<'ctx>, String> {
        ...
        if let Some(del_idx) = deleted_idx {
            ...
            let del_val =
                b!(self.bld.build_load(i64t, del_gep, "dist.del.val")).into_int_value();
    ...
    pub(in crate::codegen) fn emit_store_agg(&mut self, rest: &str, op: &str) -> Result<BasicValueEnum<'ctx>, String> {
        let parts: Vec<&str> = rest.splitn(2, "__").collect();
        if parts.len() < 2 {
        ...
        if let Some(del_idx) = deleted_idx {
            ...
            let del_val =
                b!(self.bld.build_load(i64t, del_gep, "agg.del.val")).into_int_value();
            let is_live = b!(self.bld.build_int_compare(
                inkwell::IntPredicate::EQ,
                del_val,
                i64t.const_int(0, false),
        ...
        if is_float {
            let field_val =
                b!(self.bld.build_load(f64t, field_gep, "agg.fval")).into_float_value();
            let cur_acc =
                b!(self.bld.build_load(f64t, acc_ptr, "agg.fcur")).into_float_value();
            ...
            let field_val =
                b!(self.bld.build_load(i64t, field_gep, "agg.val")).into_int_value();
        ...
        if is_float {
            let result =
                b!(self.bld.build_load(f64t, acc_ptr, "agg.fresult")).into_float_value();
            if op == "avg" {

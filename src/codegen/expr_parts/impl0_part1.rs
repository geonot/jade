...
impl<'ctx> Compiler<'ctx> {
    ...
    pub(in crate::codegen) fn compile_ref(&mut self, inner: &hir::Expr) -> Result<BasicValueEnum<'ctx>, String> {
        match &inner.kind {
            hir::ExprKind::Var(_, name) => self
                .find_var(&name.as_str())
                .map(|(ptr, _)| *ptr)
    ...
    pub(in crate::codegen) fn compile_deref(&mut self, inner: &hir::Expr) -> Result<BasicValueEnum<'ctx>, String> {
    ...
    pub(in crate::codegen) fn compile_syscall(&mut self, args: &[hir::Expr]) -> Result<BasicValueEnum<'ctx>, String> {
        if args.is_empty() {

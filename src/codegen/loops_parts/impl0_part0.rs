...
impl<'ctx> Compiler<'ctx> {
    pub(crate) fn compile_while(
        ...
        self.loop_stack.push(crate::codegen::LoopCtx {
            continue_bb: cond_bb,
            break_bb: end_bb,
        });
    ...
    pub(in crate::codegen) fn compile_for_array(
        &mut self,
        f: &hir::For,
        elem_ty: &Type,
        len: usize,
    ...
    pub(in crate::codegen) fn compile_for_vec(
        &mut self,
        f: &hir::For,
        elem_ty: &Type,
    ) -> Result<Option<BasicValueEnum<'ctx>>, String> {
    ...
    pub(in crate::codegen) fn compile_for_indexed(
        &mut self,
        f: &hir::For,
        elem_ty: &Type,
    ...
    pub(in crate::codegen) fn compile_for_string(&mut self, f: &hir::For) -> Result<Option<BasicValueEnum<'ctx>>, String> {
        let fv = self.current_fn();
        let i8t = self.ctx.i8_type();

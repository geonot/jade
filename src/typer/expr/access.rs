...
impl Typer {
    pub(in crate::typer) fn lower_expr_call(
        ...
        let _ = expected;
        match expr {
            ast::Expr::Call(callee, args, span) => {                // Generic struct/variant constructor: `T of TypeArg(args)` —
            ...
            if let ast::Expr::Ident(ctor_name, _) = callee.as_ref() {
                ...
                if (is_struct || is_variant) && !is_value {
                    let inits: Vec<ast::FieldInit> = args
                        ...
                        .collect();
                    let result = self.lower_struct_or_variant(&ctor_name.as_str(), &inits, *span)?;
                    if let Some(exp) = expected {
                        self.unify_call_result(exp, &result.ty, *span, "call result");
                    }
                    return Ok(result);
    ...
    pub(in crate::typer) fn lower_expr_method(
        ...
        let _ = expected;
        match expr {
            ast::Expr::Method(obj, method, args, span) => {                // Module-qualified dispatch: module.fn(args) → module_fn(args)
    ...
    pub(in crate::typer) fn lower_expr_field(
        ...
        let _ = expected;
        match expr {
            ast::Expr::Field(obj, field, span) => {                // Module-qualified constant access: module.CONST → module_CONST
            if let ast::Expr::Ident(ref name, _) = **obj {
                if self.modules.contains(name) && self.find_var(&name.as_str()).is_none() {
    ...
    pub(in crate::typer) fn lower_expr_index(
        ...
        let _ = expected;
        match expr {
            ast::Expr::Index(arr, idx, span) => {                let harr = self.lower_expr(arr)?;

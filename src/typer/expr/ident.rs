...
impl Typer {
    pub(in crate::typer) fn lower_expr_ident(
        ...
        let _ = expected;
        match expr {
            ast::Expr::Ident(name, span) => {                if let Some((enum_name, tag)) = self.variant_tags.get(name).cloned() {
                let is_unit = self
                    .enums
                    .get(&enum_name)
    ...
    pub(in crate::typer) fn lower_expr_qualified_ident(
        ...
        let _ = expected;
        match expr {
            ast::Expr::QualifiedIdent(type_name, variant_name, span) => {                // Handle ErrorType:Variant or EnumType:Variant

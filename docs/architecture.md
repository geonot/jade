...
A single-page tour of the Jade compiler pipeline. Updated whenever the
...
| `runtime/` | C runtime: scheduler, channels, actors, store, IO, crypto. ABI documented in `runtime/jade_rt.h`. |
| `std/` | Standard library, written in Jade. |
...
All runtime symbols are declared in `runtime/jade_rt.h`. Every `.c` file
includes that single header. Conventions:

- **Prefixes**: `jade_*` for public C API; `c_*` for non-jade C helpers; `__jade_*` for codegen-internal symbols.
...
- **Add a runtime function**: declare in `runtime/jade_rt.h`, define in the relevant `.c` file, expose to codegen via `src/runtime_ffi.rs`. See `runtime/README.md` step-by-step.
- **Add a stdlib module**: drop `std/foo.jade`, add tests in `tests/programs/std/foo.jade`.
...
- [the-way-of-jade.md](../the-way-of-jade.md) — language design philosophy.

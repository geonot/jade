# Jinn Runtime (`runtime/`)

The C runtime backing every compiled Jinn program. ~5K LOC across 28 `.c`
files plus one shared header [jinn_rt.h](jinn_rt.h). Linked statically into
each `jinnc`-produced binary as `libjinn_rt.a` (always) plus optional
`libjinn_ssl.a` (OpenSSL detected at build time) and `libjinn_sqlite.a`
(sqlite3 detected at build time).

## Conventions

- All public symbols start with `jinn_`. Helpers exposed for codegen-name
  reasons (`c_*`) wrap libc functions whose names collide with Jinn
  identifiers; `__jinn_*` are codegen-internal trampolines emitted directly
  by `src/codegen/`.
- **Resource-creating** functions return a pointer or `NULL` on failure
  (`jinn_*_open`, `jinn_*_create`, `jinn_*_prepare`).
- **Mutating** functions return `int` with `0`/positive on success and
  `-1` on failure (`c_mkdir`, `jinn_chmod`, `jinn_close`).
...
[jinn_rt.h](jinn_rt.h). Codegen call sites for each module live in the
...
| `net.c` | BSD socket wrappers (collision-free names) | `std/net.jn` |
| `pool.c` | Slab pool allocator | `src/codegen/builtins.rs` |
| `process.c` | Spawn/exec/system + Vec<String> ABI helpers | `std/process.jn` |
| `regex_helper.c` | PCRE2 ovector accessor | `std/regex.jn` |
...
| `sqlite.c` | SQLite3 binding (optional) | `std/sqlite.jn` |
...
| `tls.c` | TLS client + DNS resolver (OpenSSL) | `std/tls.jn` |
...
2. Add the prototype to [jinn_rt.h](jinn_rt.h) under the matching `runtime/X.c`
   comment block.
3. If the function needs a new opaque type, declare it as
   `typedef struct Foo Foo;` in `jinn_rt.h` and `struct Foo { … };` in the `.c`

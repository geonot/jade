 ...
 * whose names collide with Jade keywords (close, send, delete, etc.)
 */
#include <unistd.h>
...
long jade_sendto(int fd, const void *buf, long len, int flags,
                 const void *addr, int addrlen) {
    return sendto(fd, buf, (size_t)len, flags,
                  (const struct sockaddr *)addr, (socklen_t)addrlen);
}

long jade_recvfrom(int fd, void *buf, long len, int flags,
